...
Base.prototype.lock = function () {
	for (var i = 0; i < this.elements.length; i ++) {
		fixedScroll.top = getScroll().top,fixedScroll.left = getScroll().left;
	...
	for (var i = 0; i < this.elements.length; i ++) {
		...
		addEvent(window, 'resize', function () {
			...
			if (element.offsetLeft > getInner().width + getScroll().left - element.offsetWidth) {
				...
				if ( element.offsetLeft <= 0 + getScroll().left ){ element.style.left = 0 + getScroll().left + 'px'; } 
			}
			if (element.offsetTop > getInner().height + getScroll().top  - element.offsetHeight) {
				...
				if ( element.offsetTop <= 0 + getScroll().top ){ element.style.top = 0 + getScroll().top + 'px'; } 
			} 
		});
	}
	return this;
